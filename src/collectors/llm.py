...
_CLAUDE_SYS = (
    ...
    "⑧ **수급 인과를 단정하지 마라.** 확정 수치의 수급은 개인·외국인·기관(+기타법인 참고)뿐이고 "
    "프로그램·선물·상위종목 기여도는 미수집이다. 3주체 순매도인데 지수가 오른 날을 '왜곡 장세'· "
    "'환율/기술 요인에 의존' 처럼 원인 단정으로 쓰지 말고, '상승 확산 제한'·'수급 지속성 신뢰도 "
    "낮음'·'주도 요인 단정 불가(기타법인·프로그램 미수집)' 로 서술한다. "
...
def btc_facts_block(ctx: dict) -> str:
    ...
    if ctx.get("quadrant"):
        ...
        # 펀딩이 기본율 상회가 아니면(중립·하회) Q1/Q3 의 '+부호'는 군집 근거가 못 된다 — 라벨 격하.
        _ft = ctx.get("funding_txt") or ""
        if ("중립" in _ft or "하회" in _ft) and ctx.get("quadrant") in ("Q1", "Q3"):
            _qs = {"Q1": "OI↑·펀딩 중립~하회(군집 판정 불가)",
                   "Q3": "OI↓·펀딩 중립~하회(군집 판정 불가)"}[ctx["quadrant"]]
...
_BTC_CLAUDE_SYS = (
    ...
    "펀딩이 '기본율 수준=중립' 또는 '기본율 하회' 로 표기돼 있으면(기본 펀딩률 0.01%/8h 상회가 "
    "아닐 때) '펀딩+ 과열'·'레버리지 롱군집 확인'·'청산 캐스케이드 임박' 처럼 펀딩을 군집·과열의 "
    "근거로 쓰지 마라 — 그 구간의 +부호는 정보가 아니다. "
