...
def btc_facts_block(ctx: dict) -> str:
    ...
    if ctx.get("quadrant"):
        ...
        # 펀딩이 기본율 수준(중립)이면 Q1/Q3 의 '+부호'는 군집 근거가 못 된다 — 라벨 격하.
        if "중립" in (ctx.get("funding_txt") or "") and ctx.get("quadrant") in ("Q1", "Q3"):
            _qs = {"Q1": "OI↑·펀딩 중립(군집 판정 불가)",
                   "Q3": "OI↓·펀딩 중립(군집 판정 불가)"}[ctx["quadrant"]]
...
_BTC_CLAUDE_SYS = (
    ...
    "펀딩이 '기본율 수준=중립' 로 표기돼 있으면(기본 펀딩률 0.01%/8h ±0.005%p) '펀딩+ 과열'·"
    "'레버리지 롱군집 확인'·'청산 캐스케이드 임박' 처럼 펀딩을 군집·과열의 근거로 쓰지 마라 — "
    "그 구간의 +부호는 정보가 아니다. "
