...
스코어링·게이트·후보 선정에 일절 영향 없다(`src/level_scan.read_btc_pred` 가 `out/btc_latest.json`
만 읽는다. 없거나 깨지면 None → 표는 그대로).

- ✅ **게이트가 확률을 이긴다** — NO_TRADE 면 확률·차단사유만 미러하고 타점·사이즈는 애초에 안
  가져온다(payload 에 `atr`·`binance_size` 가 있어도 무시).
- ✅ **성적은 싣지 않는다** — n<40 숨김 규율. 카드는 '예측 미러'일 뿐이다.
- ✅ **신선도 정직화** — 15분마다 같은 값이 반복되는 것을 새 신호로 오인하지 않게
  'N시간 전 발행 · 다음 슬롯'을 표기한다.
- ✅ **[중] 꼬리표 하드코딩 수정** — 최초 구현이 "캘리브 표본<40·게이트 미개방 상태라"를 문자열로
  박아, 게이트가 열린 날에도 '미개방'이라 적었다(실측 09-14 22:00 `no_trade=False`·`p_long` 71.1%).
  이 세션 내내 잡던 '표시가 데이터와 어긋남'이 신규 코드에 그대로 재현된 것 — 게이트 상태와
  `accuracy.primary_n` 에서 파생하도록 고쳤다. 회귀 +2(열린 게이트·닫힌 게이트).
- ✅ **[중] 표면 검증 표기** — 「표시의 강도는 검증의 강도를 따른다」(2026-09-03 사용자 지시)를
  이 카드에도 적용. `primary_n<40` 이면 확률 숫자를 무채색으로 죽이고 사유를 ⓘ 가 아니라
  **표면**에 적는다(주식 히어로가 미검증 도넛을 무채색화하는 것과 같은 규율). 회귀 +1.

주의: 이 기능의 UI 절반이 09-14 `34e30b8` 에 잘못 쓸려 들어갔다가 `b853495` 로 되돌려졌다.
지금 커밋이 데이터·UI·테스트를 한 번에 담는 정본이다.
...
- Python은 **`.venv/bin/python`** (시스템 python3 에 pytest/httpx 없음). 테스트 **460 collected** (2026-09-15, `.venv`).
