...
> **2026-09-01 (5차) BTC 스캘핑 트랙 신설 — 리포트와 별개 유형(사용자 설계 승인):**
> 사이드바 가상화폐에 'BTC 스캘핑' 메뉴 + `render_report.build_btc_scalp_view`. **버튼을 누르는
> 순간 브라우저가 바이낸스 공개 API 직접 호출**(fapi CORS 실측·현물 data-api 폴백 — 서버·크론·
> LLM·재배포 0) → 1m·5m·15m·1h·4h 를 EMA 정렬·MACD·RSI·슈퍼트렌드·ADX 로 투표 → **상방/하방/
> 관망 3태**(비추세 ADX·상위 시간축 역행·혼조면 관망 강제 — 규칙 화면 공개). 60초 지나면 낡음
> 표시·세션 판정 기록·비용 현실 체크(5m ATR vs 왕복 0.10%/0.04%). **measure-first 동시 착수**:
> `scripts/exp_btc_scalp.py` 가 같은 판정식을 과거 60일(5m 18,144봉·15분 지평·비중첩 5,064표본)에
> 워크스루 → **상방 적중 47.0%·하방 49.0%·비용 후 평균 음수** = 현재 판정식 엣지 없음. 이 숫자를
> 뷰 '판정 성적(측정)' 카드와 버튼 아래 요약에 그대로 노출('방향 신호가 아니라 리드아웃') —
> 도구가 스스로 성적을 말한다. 리포트 트랙 스코어링·게이트와 완전 분리. 회귀 +3(총 403).

> **2026-09-01 (4차) 외부비평 라운드15 — 라벨링 레이어 정직화(점수·게이트 불변, 테스트 391→400):**
> ① 뉴스 표 카운트를 per-item 플래그에서 도출(`news._mark_votes`, 주식·BTC 공용) — 디둡 탈락
> "중복(동일 사건)"·캡 초과 "상한 초과(태그당 3)" 사유를 목록에 pill 로 표기, 라벨을
> "반영 호재 X·악재 Y · 검증 N건(48시간 내)" 로 교정('점수 반영 7건' 오표기 제거). 비평의
> '중복 이중계상' 주장 자체는 오탐(_same_event 가 ETF 쌍 잡음을 재현 확인). ② 펀딩 기본율 문맥 —
> 0.01%/8h ±0.005%p 안이면 "기본율 수준=중립" 표기, Q1/Q3 라벨 "군집 판정 불가" 격하
> (observed·렌더 타일·LLM facts·프롬프트 규칙 4곳 동기화, 쿼드런트·점수 불변). ③ 1H 약화 감지
> 확장(RSI<48 뿐 아니라 MACD hist<0·ST↓, observed 에 "1H MACD−·1H ST↓"). ④ bars 카드에 SNS 8%
> 제외 사유 명시 + 복사텍스트 가중 합 주석. ⑤ 확률 ⓘ에 판정 지평(다음 정규 슬롯 마크 등락 부호)
> 명시 · F&G 타일을 포지셔닝 카드에서 제거(SNS 카드 1곳) · 정규 슬롯 as_of 45분+ 괴리 시
> "재발행 HH:MM" 배지. ⑥ NO_TRADE 면 판정 분해 카드를 히어로 위로. 백분위 스코어링 전환은
> 잠금(관측은 gate_log 가 펀딩·OI 누적 중 — n 쌓이면 measure-first 검토).
