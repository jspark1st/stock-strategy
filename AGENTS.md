...
> **"하나"는 지금 국면이다.** 프로젝트의 **장기 목적은 AI 발전에 따라 예측 보고서를 지속 강화**하는
> 것 — 미래엔 중장기·종합 금융 보고서로 넓힌다. 하지만 **지금은 하나에 집중**한다(과거 스윙·단타·장기
> 를 섞다가 과최적화로 실패했다). 확장은 넓히는 게 아니라 **독립 트랙을 하나씩 measure-first 로
> 검증해 붙이는 것**이다 → 아래 「장기 비전」. **지금 이 락(단일 전략)은 유지된다.**
...
**목적: AI 발전에 따라 예측 보고서를 지속 강화한다.** 지금은 단기(오버나이트)지만, 미래엔
**중장기(주간·월간 방향)·종합 금융 보고서**로 넓힌다. 단, 과거 실패(스윙·단타를 한 트랙에 섞다가
과최적화)를 되풀이하지 않도록 **확장은 넓이가 아니라 트랙**이다:

- **새 보고서 = 독립 트랙**(스코어링·게이트·크론·DB·성공척도 분리). BTC 가 이미 그 증명 —
  기존 트랙에 **섞지 않는다**(절대 규칙 1 그대로). 순수 스코어링·자가학습·비평·렌더 계약은 재사용.
- **measure-first 로만 승격**(절대 규칙 2). 중장기·종합도 자기 하네스로 엣지를 측정하기 전엔 라이브
  아님. **깊이(검증) 없이 넓이(보고서 수)부터 늘리지 않는다.**
- **LLM 은 끝까지 서술·비평·리서치·개발보조**(절대 규칙 3). "AI 가 강화"의 정당한 경로 = 더 나은
  서술/비평/`/triage` 개발루프이지, **LLM 이 예측 수치를 지어내는 게 아니다**(하네스로 측정 전엔).
- **종합 보고서 = 이미 검증된 트랙들의 집계·서술**(새 예측 생성 아님). LLM 서술의 정당한 확장 영역.
- **정직성 불변**(게이트·n<40 숨김·검증 안 된 엣지 표시 금지)은 모든 신규 트랙에 그대로 적용.

> 요약: **아키텍처는 이 진화에 준비돼 있다**(순수 스코어링·독립 트랙·하네스·자가학습·비평 루프·
> 다중뷰 셸). 위험은 오직 **규율을 버리고 넓이부터 늘리는 것**. 강화의 병목은 코드가 아니라
> **데이터·검증**이다. 상세 로드맵 → [`guide_docs/roadmap/README.md`](guide_docs/roadmap/README.md) 「L5+」.
...
PYTHONUTF8=1 .venv/bin/python -m pytest tests/ -q                  # 364 collected (2026-08-31)
```
