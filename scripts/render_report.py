...
구조: 좌측 사이드바(시장 그룹: 코스피/코스닥, 아이템: 장 마감 전·후 분석 / 개장전 분석) + 우측 뷰.
...
LABEL_CLOSE = "장 마감 전·후 분석"   # 15:00 잠정(마감 전)·16:30 확정(마감 후) 둘 다 담는 뷰
...
def build_level_scan_view() -> str:
    ...
    <div class="card" id="btc-pred-card" hidden>
      ...
      <div class="note muted" id="btc-pred-unproven" hidden></div>
    ...
    <div class="card">
      ...
      <div class="note muted" id="scan-stale" hidden>표의 수치가 20분 넘게 갱신되지 않았습니다 — 잠시 뒤 다시 확인해 주세요.</div>
      ...
      <div style="overflow-x:auto">
        <table class="cd-table" id="scan-table">
          ...
          <tbody id="scan-body">
            <tr><td colspan="16" class="muted">아직 표시할 종목이 없습니다 — 15분마다 자동으로 채워집니다.</td></tr>
          </tbody>
        </table>
      </div>
    </div>
    ...
    (function(){{
      ...
      function paintBtc(b){{
        ...
        // 표시의 강도는 검증의 강도를 따른다 — 주 지평 표본이 모자라면 색을 빼고 표면에 적는다.
        var pn=b.primary_n, unproven=(pn==null||pn<40);
        var elU=document.getElementById('btc-pred-unproven');
        if(elL) elL.classList.toggle('btc-pred-dim', unproven);
        if(elS) elS.classList.toggle('btc-pred-dim', unproven);
        if(elU){{
          elU.hidden=!unproven;
          if(unproven){{
            var cnt=(pn==null)?'미상':(pn+'건');
            elU.textContent='주 지평(종가→시가) 검증 표본 '+cnt+' — 검증된 엣지가 아닙니다. '
              +'아래 표의 후보를 자르는 기준으로 쓰지 마세요.';
          }}
        }}
...
def build_btc_scalp_view() -> str:
    ...
    if meas:
        ...
        meas_html = (
            ...
            # 2026-09-01 확장 측정(60~120일·반쪽 분할 검증) 요약 — 3계열 전부 음성.
            # 되돌림 방향성은 재현되지만 판당 총이익이 최저 비용(메이커 0.04%)조차 못 넘는다.
            f'<div class="note muted">확장 측정: 판정을 <b>역방향</b>으로 쓰면 52~55%, 급변 이벤트'
            f'(테이커 불균형·점화·급변) 직후 되돌림은 53~61%로 <b>방향성 자체는 재현</b>되지만, '
            f'세 계열 모두 판당 이익이 메이커 수수료(0.04%)에 못 미쳐 <b>매매 가치는 없다</b> '
            f'(exp_btc_scalp / exp_btc_scalp_events · 반쪽 분할 검증). 트리플 배리어(±ATR 익절·'
            f'손절·1h 제한·스파이크 슬리피지) 청산 관리로 재채점해도 42개 조합 전원 음수, '
            f'프로 도구 2차(VWAP 리버전·CVD 다이버전스·전일 고저·시간대·펀딩 극단)도 '
            f'별도 기간 OOS 에서 생존 0 — 단기 되돌림은 실재하나 리테일 비용 아래에 있다. '
            f'그래서 이 도구는 방향 신호가 아니라 상태 계기판이다.</div>')
    else:
    ...
    <div class="card" id="scalp-deriv-card" hidden><h2>파생 수급 데이터{_info("바이낸스가 제공하는 파생 시장 데이터를 시간프레임별로 정리했습니다. 각 칸은 그 시간프레임의 최근 구간(API 제공분) 평균 대비 z-score(σ)입니다 — 5분은 최근 ~1.7일, 30분 ~10일, 1시간 ~21일, 4시간·12시간 ~30일 기준. 극단(±2σ)일수록 역발상 주의: 과다 롱은 롱 스퀴즈, 과다 숏은 숏 스퀴즈 위험. 여러 시간프레임에서 동시에 벌어질수록 쏠림이 뚜렷합니다. 점수·판정에 반영되지 않는 관측 지표이며, 평균은 API 가 주는 짧은 구간 기준이라 잠정적입니다.")}</h2>
      <div style="overflow-x:auto"><table class="cd-table" id="scalp-deriv"><thead><tr><th>지표</th><th>현재</th><th>5분</th><th>30분</th><th>1시간</th><th>4시간</th><th>12시간</th><th>최대 이탈 · 해석</th></tr></thead><tbody></tbody></table></div>
      <div class="note muted" id="scalp-deriv-note">칸 = 해당 시간프레임 평균 대비 z-score(σ) · 바이낸스 제공 구간 상한 30일 · 점수 미반영 관측 · 극단(±2σ)은 역발상 참고.</div></div>
    ...
    (function(){
      ...
      var DERIV_TFS=[['5m','5분'],['30m','30분'],['1h','1시간'],['4h','4시간'],['12h','12시간']];
      var DERIV_MET=[
        {n:'미결제약정(OI)',ep:'openInterestHist',get:function(x){return +x.sumOpenInterest;},
         fmt:function(x){return Math.round(x/1000).toLocaleString()+'K';},u:' BTC',
         hi:'증가=신규 진입 유입',lo:'감소=포지션 청산'},
        {n:'롱숏 비율(전체 계정)',ep:'globalLongShortAccountRatio',get:function(x){return +x.longShortRatio;},
         fmt:function(x){return x.toFixed(3);},u:'',
         hi:'롱 과다 → 롱 스퀴즈 주의(역발상)',lo:'숏 과다 → 숏 스퀴즈 주의(역발상)'},
        {n:'롱숏 비율(상위 트레이더)',ep:'topLongShortPositionRatio',get:function(x){return +x.longShortRatio;},
         fmt:function(x){return x.toFixed(3);},u:'',
         hi:'상위 롱 편중',lo:'상위 숏 편중'},
        {n:'테이커 매수/매도',ep:'takerlongshortRatio',get:function(x){return +x.buySellRatio;},
         fmt:function(x){return x.toFixed(3);},u:'',
      ...
      function loadDeriv(){
        ...
        Promise.all(jobs).then(function(res){
          var by={};res.forEach(function(r){by[r.ep+'|'+r.p]=r.s;});
          var html='';
          for(var i=0;i<DERIV_MET.length;i++){var m=DERIV_MET[i],cur=null,best=null,cells='';
            for(var j=0;j<DERIV_TFS.length;j++){var s=by[m.ep+'|'+DERIV_TFS[j][0]];
              if(s){if(cur==null)cur=s.last;var z=s.sd>0?(s.last-s.m)/s.sd:0;cells+='<td>'+zbadge(z)+'</td>';
                if(best==null||Math.abs(z)>Math.abs(best.z))best={z:z,tf:DERIV_TFS[j][1]};}
              else cells+='<td class="muted">—</td>';}
            var interp=best?(Math.abs(best.z)<1?'<span class="muted">전 구간 평균 범위</span>':
              best.tf+' '+(best.z>0?'+':'')+best.z.toFixed(1)+'σ · '+(best.z>0?m.hi:m.lo)):'<span class="muted">데이터 부족</span>';
            var curTxt=cur!=null?'<b>'+m.fmt(cur)+m.u+'</b>':'<span class="muted">—</span>';
            html+='<tr><td>'+m.n+'</td><td>'+curTxt+'</td>'+cells+'<td>'+interp+'</td></tr>';}
...
_NAV_REMAP = {
    ...
    # 구 라벨(장마감전 분석)로 저장된 아카이브/번들도 새 라벨로 리라벨 — 화면 일관.
    ("코스피", "장마감전 분석"): ("코스피", LABEL_CLOSE),
    ("코스닥", "장마감전 분석"): ("코스닥", LABEL_CLOSE),
}
...
<style>
  ...
  *{box-sizing:border-box;margin:0;padding:0}
  ...
  .btc-pred-ls.btc-pred-dim b{color:var(--muted)}
  ...
  /* flex-wrap: 좁으면 '제외' 배지를 다음 줄로 내린다. overflow-wrap:break-word: .card 의
     overflow-wrap:anywhere 가 flex 항목(제목)의 min-content 를 1글자로 만들어 nowrap 배지가
     너비를 다 먹을 때 제목이 세로 1글자씩 깨지던 모바일 버그 방지(min-content=최장 단어). */
  .mat-ul li{display:flex;gap:8px;align-items:flex-start;flex-wrap:wrap;overflow-wrap:break-word}
